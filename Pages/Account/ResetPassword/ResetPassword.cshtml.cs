...
namespace CompanyEmployees.IDP.Pages.Account.ResetPassword
{
    ...
    public class ResetPasswordModel : PageModel
    {
        ...
        {
            Input = new Entities.ViewModels.ResetPasswordModel
            {
                Token = token,
                Email = email
            };
        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            if (user == null)
                RedirectToPage("/Account/ResetPassword/ResetPasswordConfirmation", new
                {
                    ReturnUrl
                });
            var resetPassResult = await _userManager.ResetPasswordAsync(user, Input.Token,
           Input.Password);
